 ...
 */
export const reportResults = (cwd, packResult, config) => {
    logger.log('');
    if (config.output.topFilesLength > 0) {
        reportTopFiles(packResult.fileCharCounts, packResult.fileTokenCounts, config.output.topFilesLength, packResult.totalTokens);
        logger.log('');
    }
    ...
    reportSecurityCheck(cwd, packResult.suspiciousFilesResults, packResult.suspiciousGitDiffResults, packResult.suspiciousGitLogResults, config);
    logger.log('');
...
export const reportSummary = (packResult, config) => {
    var _a, _b;
    let securityCheckMessage = '';
    if (config.security.enableSecurityCheck) {
        if (packResult.suspiciousFilesResults.length > 0) {
            securityCheckMessage = pc.yellow(`${packResult.suspiciousFilesResults.length.toLocaleString()} suspicious file(s) detected and excluded`);
        }
        else {
    ...
    if ((_a = config.output.git) === null || _a === void 0 ? void 0 : _a.includeDiffs) {
        ...
        if (packResult.gitDiffTokenCount) {
            gitDiffsMessage = pc.white(`✔ Git diffs included ${pc.dim(`(${packResult.gitDiffTokenCount.toLocaleString()} tokens)`)}`);
        }
        else {
    ...
    if ((_b = config.output.git) === null || _b === void 0 ? void 0 : _b.includeLogs) {
        let gitLogsMessage = '';
        if (packResult.gitLogTokenCount) {
            gitLogsMessage = pc.white(`✔ Git logs included ${pc.dim(`(${packResult.gitLogTokenCount.toLocaleString()} tokens)`)}`);
        }
        else {
...
export const reportSecurityCheck = (rootDir, suspiciousFilesResults, suspiciousGitDiffResults, suspiciousGitLogResults, config) => {
    if (!config.security.enableSecurityCheck) {
        return;
    }
...
const reportSuspiciousGitContent = (title, results) => {
    if (results.length === 0) {
        return;
    }
    logger.log('');
...
export const reportTopFiles = (fileCharCounts, fileTokenCounts, topFilesLength, totalTokens) => {
    ...
    filesWithTokenCounts.forEach(([filePath, tokenCount], index) => {
        ...
        logger.log(`${pc.white(`${indexString}`)} ${pc.white(filePath)} ${pc.dim(`(${tokenCount.toLocaleString()} tokens, ${charCount.toLocaleString()} chars, ${percentageOfTotal}%)`)}`);
    });
};
export const reportSkippedFiles = (_rootDir, skippedFiles) => {
...
//# sourceMappingURL=cliReport.js.map