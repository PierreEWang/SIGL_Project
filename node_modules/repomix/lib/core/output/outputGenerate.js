...
export const buildOutputGeneratorContext = (rootDirs_1, config_1, allFilePaths_1, processedFiles_1, ...args_1) => __awaiter(void 0, [rootDirs_1, config_1, allFilePaths_1, processedFiles_1, ...args_1], void 0, function* (rootDirs, config, allFilePaths, processedFiles, gitDiffResult = undefined, gitLogResult = undefined, deps = {
    listDirectories,
    listFiles,
    searchFiles,
}) {
    ...
    const shouldUseFullTree = config.output.directoryStructure === true &&
        !!config.output.includeFullDirectoryStructure &&
        ((_b = (_a = config.include) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0) > 0;
    // Paths to include in the directory tree visualization
    let directoryPathsForTree = [];
    let filePathsForTree = allFilePaths;
    if (shouldUseFullTree) {
        try {
            // Collect all directories and all files from all roots
            const [allDirectoriesByRoot, allFilesByRoot] = yield Promise.all([
            ...
            throw new RepomixError(`Failed to build full directory structure: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? { cause: error } : undefined);
        }
    }
        ...
        try {
            const merged = (yield Promise.all(rootDirs.map((rootDir) => deps.searchFiles(rootDir, config)))).reduce((acc, curr) => ({
            ...
            throw new RepomixError(`Failed to search for empty directories: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? { cause: error } : undefined);
        }
    }
    return {
